...
def _load_custom_adapters() -> None:
    from . import adapters_custom

    for module_info in pkgutil.iter_modules(adapters_custom.__path__):
        module = importlib.import_module(f"{adapters_custom.__name__}.{module_info.name}")
...
_load_custom_adapters()

# Keep explicit registrations as the static validator's source of truth while
# the dynamic loader remains the runtime extension mechanism.
from .adapters_custom.dxy import DxyAdapter
from .adapters_custom.earth_knowledge import EarthKnowledgeAdapter
from .adapters_custom.gelong import GelongAdapter
from .adapters_custom.leviathan import LeviathanAdapter
from .adapters_custom.simple_psychology import SimplePsychologyAdapter

ADAPTERS.update(
    {
        "simple_psychology": SimplePsychologyAdapter,
        "dxy": DxyAdapter,
        "earth_knowledge": EarthKnowledgeAdapter,
        "gelong": GelongAdapter,
        "leviathan": LeviathanAdapter,
    }
)
